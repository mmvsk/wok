...
case $action in
	...
	add)
		...
		echo -n "Creating mongo user and database $user... "
			cmd="
				use $db
				db.addUser('$user', '$password');
			"
			echo "$cmd" | silent mongo -u root -p$wok_mongo_passwd admin
			echo "done"
		touch $repo/$domain
	...
	rm)
		...
		if test ! $force; then
			confirm "Remove MongoDB user and database?" || exit 0
		fi
		source $repo/$domain
		echo -n "Removing user and database... "
			cmd="
				use $_db
				db.dropDatabase();
			"
